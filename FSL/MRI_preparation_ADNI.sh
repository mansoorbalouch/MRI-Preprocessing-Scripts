...
# This script gathers/collects the T1-w MRIs in a single directory with a sub-dir for each subject following the Subject-ID naming convention 
# Copies the T1-w MRIs (with MPRAGE protocol only) in dcm format
# Discards and retains only single-session files when multiple sessions exist for the subject
# Creates sub-directories for each Subject, name with Subject-ID 
